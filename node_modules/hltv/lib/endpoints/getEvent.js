        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, utils_1.fetchPage("https://www.hltv.org/events/" + id + "/" + utils_1.generateRandomSuffix(), config.loadPage)];
                case 1:
                    ...
                    relatedEvents = $('.related-event')
                        ...
                        id: el.find('a').attrThen('href', utils_1.getIdAt(2))
                    }); });
                    prizeDistribution = $('.placement')
                        .toArray()
                        .map(function (el) {
                        var otherPrize = el.find('.prizeMoney').first().next().text() || undefined;
                        ...
                        return {
                            ...
                            team: el.find('.team').children().exists()
                                ? {
                                    ...
                                    id: el.find('.team a').attrThen('href', utils_1.getIdAt(2))
                                }
                                : undefined
                        };
                    });
                    ...
                    teams = $('.team-box')
                        ...
                        return {
                            ...
                            id: el.find('.team-name a').attrThen('href', utils_1.getIdAt(2)),
                            reasonForParticipation: el.find('.sub-text').trimText(),
                            rankDuringEvent: utils_1.parseNumber(el.find('.event-world-rank').text().replace('#', ''))
                        };
                    })
                    ...
                    mapPool = $('.map-pool-map-holder')
                        .toArray()
                        .map(function (el) { return GameMap_1.fromMapName(el.find('.map-pool-map-name').text()); });
                    highlights = $('.highlight-video')
                        ...
                        var thumbnail = thumbnailBase + "-preview.jpg";
                        var team1Name = el
                            .find('.video-team')
                            .first()
                            .find('.video-team-img')
