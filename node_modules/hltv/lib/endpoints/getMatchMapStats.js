        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0: return [4 /*yield*/, Promise.all([
                        utils_1.fetchPage("https://www.hltv.org/stats/matches/mapstatsid/" + id + "/-", config.loadPage).then(scraper_1.HLTVScraper),
                        utils_1.fetchPage("https://www.hltv.org/stats/matches/performance/mapstatsid/" + id + "/-", config.loadPage).then(scraper_1.HLTVScraper)
                    ])];
                case 1:
                    _b = _c.sent(), m$ = _b[0], p$ = _b[1];
                    matchId = m$('.match-page-link').attrThen('href', utils_1.getIdAt(2));
                    ...
                    map = GameMap_1.fromMapName(m$('.match-info-box').contents().eq(3).trimText());
                    date = m$('.match-info-box span[data-time-format]').numFromAttr('data-unix');
                    team1 = {
                        id: m$('.team-left a').attrThen('href', utils_1.getIdAt(3)),
                        name: m$('.team-left .team-logo').attr('title')
                    };
                    team2 = {
                        id: m$('.team-right a').attrThen('href', utils_1.getIdAt(3)),
...
function getRoundHistory($, team1, team2) {
    ...
    var topLeftHalf = $('.round-history-half')
        .eq(0)
        .find('img')
        .toArray()
        .map(getOutcome);
    var bottomLeftHalf = $('.round-history-half')
        .eq(2)
        .find('img')
        .toArray()
        .map(getOutcome);
    var topRightHalf = $('.round-history-half')
        .eq(1)
        .find('img')
        .toArray()
        .map(getOutcome);
    var bottomRightHalf = $('.round-history-half')
        .eq(3)
        .find('img')
        .toArray()
        .map(getOutcome);
    var team1Outcomes = topLeftHalf.concat(topRightHalf);
    var team2Outcomes = bottomLeftHalf.concat(bottomRightHalf);
    var doesTeam1StartAsCt = topLeftHalf.some(function (x) { return x.outcome.includes('ct'); });
...
function getStatsOverview($) {
    ...
    var mostX = $('.most-x-box')
        ...
        res[prop] = {
            id: playerHref ? utils_1.getIdAt(3, playerHref) : undefined,
...
function getPlayerStats(m$, p$) {
    ...
    var getPlayerOverviewStats = function (el) {
        ...
        return __assign(__assign({ player: {
                ...
                .find('.st-kdratio')
                .textThen(function (x) { return utils_1.parseNumber(x.replace('%', '')); }), killDeathsDifference: el.find('.st-kddiff').numFromText(), ADR: el.find('.st-adr').numFromText(), firstKillsDifference: el.find('.st-fkdiff').numFromText() }, (el.find('.st-rating .ratingDesc').text() === '2.0'
            ? { rating2: rating }
