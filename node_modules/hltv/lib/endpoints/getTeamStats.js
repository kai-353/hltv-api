...
var getTeamStats = function (config) { return function (options) { return __awaiter(void 0, void 0, void 0, function () {
    var query, $, _a, name, currentTeam, currentLineup, currentRosterQuery, _b, historicPlayers, standins, _c, m$, e$, mp$, overviewStats, _d, wins, draws, losses, overview, matches, events, getMapStat, mapStats;
    return __generator(this, function (_e) {
        switch (_e.label) {
            case 0:
                query = querystring_1.stringify(__assign(__assign(__assign(__assign(__assign(__assign({}, (options.startDate ? { startDate: options.startDate } : {})), (options.endDate ? { endDate: options.endDate } : {})), (options.matchType ? { matchType: options.matchType } : {})), (options.rankingFilter ? { rankingFilter: options.rankingFilter } : {})), (options.maps ? { maps: options.maps.map(GameMap_1.toMapFilter) } : {})), (options.bestOfX ? { bestOfX: options.bestOfX } : {})));
                _a = scraper_1.HLTVScraper;
                return [4 /*yield*/, utils_1.fetchPage("https://www.hltv.org/stats/teams/" + options.id + "/-?" + query, config.loadPage)];
            case 1:
                ...
                currentRosterQuery = querystring_1.stringify({
                    lineup: currentLineup.map(function (x) { return x.id; }),
                    minLineupMatch: 0
                });
                if (!options.currentRosterOnly) return [3 /*break*/, 3];
                _b = scraper_1.HLTVScraper;
                return [4 /*yield*/, utils_1.fetchPage("https://www.hltv.org/stats/lineup?" + currentRosterQuery, config.loadPage)];
            case 2:
            ...
            case 3:
                ...
                return [4 /*yield*/, Promise.all([
                        utils_1.fetchPage(options.currentRosterOnly
                            ? "https://www.hltv.org/stats/lineup/matches?" + currentRosterQuery + "&" + query
                            : "https://www.hltv.org/stats/teams/matches/" + options.id + "/" + utils_1.generateRandomSuffix() + "?" + query, config.loadPage).then(scraper_1.HLTVScraper),
                        utils_1.fetchPage(options.currentRosterOnly
                            ? "https://www.hltv.org/stats/lineup/events?" + currentRosterQuery + "&" + query
                            : "https://www.hltv.org/stats/teams/events/" + options.id + "/" + utils_1.generateRandomSuffix() + "?" + query, config.loadPage).then(scraper_1.HLTVScraper),
                        utils_1.fetchPage(options.currentRosterOnly
                            ? "https://www.hltv.org/stats/lineup/maps?" + currentRosterQuery + "&" + query
                            : "https://www.hltv.org/stats/teams/maps/" + options.id + "/" + utils_1.generateRandomSuffix() + "?" + query, config.loadPage).then(scraper_1.HLTVScraper)
                    ])];
            case 4:
                ...
                matches = m$('.stats-table tbody tr')
                    ...
                    return {
                        ...
                        team1: currentTeam,
                        team2: {
                            id: el.find('img.flag').parent().attrThen('href', utils_1.getIdAt(3)),
                            name: el.find('img.flag').parent().trimText()
                        },
                        map: GameMap_1.fromMapName(el.find('.statsMapPlayed').text()),
                        mapStatsId: el.find('.time a').attrThen('href', utils_1.getIdAt(4)),
                ...
                mapStats = mp$('.two-grid .col .stats-rows')
                    ...
                    var mapName = GameMap_1.fromMapName(mapEl.prev().find('.map-pool-map-name').text());
...
function getPlayersByContainer(container) {
    return container
        ...
        id: el.attrThen('href', utils_1.getIdAt(3)),
