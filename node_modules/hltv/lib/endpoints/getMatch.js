        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, utils_1.fetchPage("https://www.hltv.org/matches/" + id + "/" + utils_1.generateRandomSuffix(), config.loadPage)];
                case 1:
                    ...
                    playerOfTheMatch = getPlayerOfTheMatch($);
...
function getVetoes($, team1, team2) {
    var getVeto = function (text) {
        ...
        if (!map || !teamName) {
            return {
                map: GameMap_1.fromMapName(text.split(' ')[1]),
                type: 'leftover'
            };
        }
        return {
            team: [team1, team2].find(function (t) { return t.name === teamName.trim(); }),
            map: GameMap_1.fromMapName(map.trim()),
...
function getEvent($) {
    return {
        ...
        id: $('.timeAndEvent .event a').attrThen('href', utils_1.getIdAt(2))
    };
}
function getOdds($) {
    return $('[class^="world"] tr.provider:not(.hidden)')
        ...
        return {
            ...
            team2: convertOdds ? utils_1.percentageToDecimalOdd(oddTeam2) : oddTeam2
        };
    });
}
function getCommunityOdds($) {
    if ($('.pick-a-winner').exists()) {
        return {
            provider: 'community',
            team1: utils_1.percentageToDecimalOdd(Number($('.pick-a-winner-team.team1 > .percentage')
                .first()
                .text()
                .replace('%', ''))),
            team2: utils_1.percentageToDecimalOdd(Number($('.pick-a-winner-team.team2 > .percentage')
                .first()
                .text()
                .replace('%', '')))
        };
...
function getMaps($) {
    return $('.mapholder')
        ...
        return {
            name: GameMap_1.fromMapName(mapEl.find('.mapname').text()),
            result: result,
            statsId: statsId
        };
    });
        ...
        ? {
            team1: {
                ...
                id: $('.lineups-compare-left .lineups-compare-player-links a')
                    .first()
                    .attrThen('href', utils_1.getIdAt(2))
            },
            team2: {
    ...
    return $('.head-to-head-listing tr')
        ...
        if (!isDraw) {
            winner = {
                ...
                id: matchEl.find('.winner .flag').next().attrThen('href', utils_1.getIdAt(2))
            };
        }
        var event = {
            name: matchEl.find('.event a').text(),
            id: matchEl.find('.event a').attrThen('href', utils_1.getIdAt(2))
        };
...
function getStatsId($) {
    ...
    if (statsEl.exists() && !statsEl.attr('href').includes('mapstats')) {
        return utils_1.getIdAt(3, $('.stats-detailed-stats a').attr('href'));
    }
}
function getPlayerOfTheMatch($) {
    var playerLink = $('.highlighted-player')
        .find('.flag')
        .next()
        .attr('href');
    return playerLink
